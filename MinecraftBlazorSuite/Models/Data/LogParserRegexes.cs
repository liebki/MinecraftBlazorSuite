...
public static partial class LogParserRegexes
{
    ...
    [GeneratedRegex(@"\[(?<loggingTime>\d+:\d+:\d+)\] \[.*\]: Loading (?<spongeServer>[A-Za-z])+, please wait\.\.\.", RegexOptions.Compiled)]
    private static partial Regex SpongeServerDetection();
    
    [GeneratedRegex(@"\[(?<loggingTime>\d+:\d+:\d+)\] \[.*\]: This server is running (?<craftbukkit>[A-Za-z]+) version (?<craftbukkitServerType>[A-Za-z0-9]+(-[A-Za-z0-9]+)+) \(MC: (?<mcVersion>[0-9]+(\.[0-9]+)+)\) \((?<spigotApiVersion>[^)]*\))", RegexOptions.Compiled)]
    ...
    [GeneratedRegex("\\[(?<loggingTime>\\d+:\\d+:\\d+)\\] \\[.*\\]: Done \\((?<startupDuration>[0-9]*\\.[0-9]+)s\\)! For help, type \"help\"", RegexOptions.Compiled)]
    private static partial Regex ServerStartupDoneDetection();
    
    [GeneratedRegex(@"\[(?<loggingTime>\d+:\d+:\d+)\] \[.*\]: UUID of player (?<playerName>\w+) is (?<playerUuid>[\w-]+)", RegexOptions.Compiled)]
    private static partial Regex PlayerJoinedServerDetection();
    
    [GeneratedRegex(@"\[(?<loggingTime>\d+:\d+:\d+)\] \[.*\]: Starting minecraft server version (?<minecraftVersion>[0-9].+)", RegexOptions.Compiled)]
    private static partial Regex ServerVersionDetection();
    
    [GeneratedRegex(@"\[(?<loggingTime>\d+:\d+:\d+)\] \[.*\]: (?<playerName>\w+)\[(?<playerAddress>[^\]]+)\] logged in with entity id \d+ at \((\[[^\]]+\])?(.*?)\)", RegexOptions.Compiled)]
    private static partial Regex PlayerServerLoginDetection();
    
    [GeneratedRegex(@"\[(?<loggingTime>\d+:\d+:\d+)\] \[.*\]: <(?<playerName>\w+)> (?<chatMessage>.+)", RegexOptions.Compiled)]
    private static partial Regex PlayerServerChatDetection();
    
    [GeneratedRegex(@"\[(?<loggingTime>\d+:\d+:\d+)\] \[.*\]: (?<playerName>\w+) issued server command: (?<commandMessage>.+)", RegexOptions.Compiled)]
    private static partial Regex PlayerServerCommandExecDetection();
    
    [GeneratedRegex(@"\[(?<loggingTime>\d+:\d+:\d+)\] \[.*\]: (?<playerName>\w+) lost connection: Disconnected", RegexOptions.Compiled)]
    ...
    [GeneratedRegex(@"\[(?<loggingTime>\d+:\d+:\d+)\] \[.*\]: (?<playerName>\w+) joined the game", RegexOptions.Compiled)]
    private static partial Regex PlayerServerJoinDetection();
    
    [GeneratedRegex(@"\[(?<loggingTime>\d+:\d+:\d+)\] \[.*\]: There are (?<playerCount>\d+) of a max of (?<maxPlayerCount>\d+) players online: (?<playerNames>.+)", RegexOptions.Compiled)]
