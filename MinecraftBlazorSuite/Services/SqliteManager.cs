...
namespace MinecraftBlazorSuite.Services
{
    ...
    public class SqliteManager
    {
        ...
        private const string UserTableQuery = "CREATE TABLE IF NOT EXISTS users (id INTEGER NOT NULL PRIMARY KEY, username varchar, password_hash varchar, rights int, last_login_ip varchar)";
        ...
        public async Task<int> CreateUser(string username, string password, PanelUserRights rights, string lastLoginIp)
        {
            ...
            cmd.CommandText = "INSERT INTO users (username, password_hash, rights, last_login_ip) VALUES (@username, @passwordHash, @rights, @lastLoginIp)";
        ...
        public async Task UpdateUser(int userId, string username, string newPassword, PanelUserRights newRights, string newLastLoginIp)
        {
            using SqliteConnection con = GetCon();
            con.Open();
            ...
            cmd.CommandText = "UPDATE users SET password_hash = @passwordHash, rights = @rights, last_login_ip = @lastLoginIp WHERE id = @userId";
