...
namespace MinecraftBlazorSuite.Pages
{
    partial class PlayersView
    {
        ...
        {
            SendCommand(giveRights ? string.Format(MinecraftCommands.OpCommand, playername) : string.Format(MinecraftCommands.DeopCommand, playername));
        }
        ...
        {
            IDialogReference dialog = await DialogService.ShowAsync<PlayerMessageInputDialog>($"Kick player {playername} because:");
        ...
        {
            IDialogReference dialog = await DialogService.ShowAsync<PlayerMessageInputDialog>($"Ban player {playername} because:");
            DialogResult result = await dialog.Result;
        ...
        {
            IDialogReference dialog = await DialogService.ShowAsync<PlayerMessageInputDialog>($"Send message to {playername}");
            DialogResult result = await dialog.Result;
        ...
        {
            IDialogReference dialog = await DialogService.ShowAsync<PlayerChangeGamemodeDialog>($"Change the gamemode of {playername}");
            DialogResult result = await dialog.Result;
